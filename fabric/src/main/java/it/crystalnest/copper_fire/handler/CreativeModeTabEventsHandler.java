  ...
  public static void handle(CreativeModeTab tab, FabricCreativeModeTabOutput entries) {
    BuiltInRegistries.CREATIVE_MODE_TAB.getResourceKey(tab).ifPresent(key -> {
      if (key == CreativeModeTabs.FUNCTIONAL_BLOCKS) {
        entries.insertAfter(Items.CAMPFIRE, FireManager.getRequiredComponent(FireManager.COPPER_FIRE_TYPE, Fire.Component.CAMPFIRE_ITEM));
      } else if (key == CreativeModeTabs.INGREDIENTS || key == CreativeModeTabs.TOOLS_AND_UTILITIES) {
        entries.insertAfter(Items.FIRE_CHARGE, FireManager.getRequiredComponent(FireManager.COPPER_FIRE_TYPE, Fire.Component.FIRE_CHARGE_ITEM));
      }
    });
  }
}
